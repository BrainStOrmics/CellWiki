# =============================================================================
# 查询 LangGraph 子图 —— 多阶段检索：实体匹配 → 图谱扩展 → 上下文组装 → 综合
# =============================================================================
# 定义于 MASTER_PLAN.md 第 4.3 节。
# 多阶段检索流程：实体匹配 → 图谱扩展 → 上下文组装 → 综合回答。
# 支持实体识别、知识图谱扩展、上下文窗口预算管理和引用追踪。
# =============================================================================
...
from cellwiki.legacy.graphs.states import QueryState
...
def parse_question(state: QueryState) -> dict:
    ...
    # Deduplicate: same entity may be caught by both pattern and keyword matching
    seen = set()
    unique_entities = []
...
def entity_search(state: QueryState) -> dict:
    ...
    for entity in entities:
        if entity["type"] == "cell_type":
            ...
            # Marker gene directory is optional; not all wikis populate it
    ...
    # Deduplicate by page ID — multiple entities may resolve to the same page
    seen = set()
    unique_matches = []
    for m in matches:
        if m["id"] not in seen:
...
def route_after_search(state: QueryState) -> str:
    """Route to research subgraph if no local matches, otherwise expand graph."""
...
def graph_expansion(state: QueryState) -> dict:
    ...
    # Load pre-computed relationship edges to expand from matched pages
    from cellwiki.config import settings
    ...
    if settings.wiki_cell_types_dir.exists():
        for page_id in initial:
            ...
            if page.exists():
                ...
                # Wikilinks [[page]] encode semantic relationships between pages
                links = re.findall(r'\[\[(.+?)\]\]', content)
                for link in links:
                    if link not in expanded:
                        expanded.append(link)

    # Hard limit prevents context window overflow
...
def budget_select(state: QueryState) -> dict:
    ...
    for page_id in expanded:
        ...
        if used + tokens > max_tokens and selected:
            # Stop early once we have at least one page and adding the next would exceed budget
            return {
