# =============================================================================
# CLI 入口 —— CellWiki 命令行界面
# =============================================================================
# 分发到子命令用于论文导入、细胞类型提取、Wiki 生成、审计和
# LangGraph 驱动的智能体工作流。CLI 是 CellWiki 系统的主要用户入口。
# 支持 v1（传统）和 v2（智能体）两种工作流模式。
# =============================================================================

#!/usr/bin/env python3
"""CellWiki CLI - Build and manage a single-cell biology knowledge base.

Dispatches to subcommands for paper ingestion, cell-type extraction, wiki
generation, auditing, and LangGraph-powered agentic workflows.  The CLI is
the primary user-facing entry point for the CellWiki system.
"""
...
try:
    ...
    from cellwiki.legacy.graph_analysis import build_and_analyze, CellWikiGraph
    HAS_LANGGRAPH = True
except ImportError:
    HAS_LANGGRAPH = False
...
def _collect_pdfs(paths: list[Path]) -> list[Path]:
    ...
    for p in paths:
        if p.is_dir():
            # Recursively gather PDFs so users can pass a directory tree
            pdfs.extend(sorted(p.rglob("*.pdf")))
        elif p.is_file() and p.suffix.lower() == ".pdf":
            pdfs.append(p)
        else:
            # Warn for non-PDF paths rather than silently ignoring
...
def cmd_add(args):
    ...
    for pdf_path in pdfs:
        ...
        if not dest.exists():
            ...
            # Skip duplicate copies to avoid re-processing identical files
            print(f"\n{'=' * 60}")
...
def cmd_query(args):
    ...
    # Windows GBK terminal cannot render many Unicode characters; reconfigure
    # stdout to use UTF-8 with replacement so Rich and print() do not crash.
    ...
    if not page.exists():
        # Fuzzy match: partial substring or underscore-normalized equality
    ...
    console = Console(width=120, file=sys.stdout)
    with open(page, encoding="utf-8") as f:
        content = f.read()
    # Strip YAML frontmatter before rendering Markdown to the user
...
def cmd_diff(args):
    ...
    # Set diff: new types appear, shared types are augmented
...
def cmd_graph(args):
    ...
    ensure_dirs()
    from cellwiki.legacy.visualization import generate_relationship_graph
    ...
    for p in wiki_pages:
        with open(p, encoding="utf-8") as f:
            ...
            if content.startswith("---"):
                ...
                # CL:0000000 is the "unresolved / not yet mapped" sentinel
...
def cmd_audit(args):
    ...
    from cellwiki.legacy.audit import run_audit, display_audit_table, generate_audit_report
...
def cmd_review(args):
    ...
    if missing_cl:
        ...
        for item in missing_cl:
            ...
            candidates = []
            # Heuristic: match against synonym_map by splitting on whitespace/cell
    ...
    if corrections:
        ...
        # Load existing corrections to merge rather than overwrite
...
def _collect_source_files(paths: list[Path]) -> list[Path]:
    """Collect supported source files for the Agentic v1 path."""
    supported = {".pdf", ".md", ".txt"}
    files = []
    for path in paths:
        if path.is_dir():
            files.extend(sorted(p for p in path.rglob("*") if p.is_file() and p.suffix.lower() in supported))
        elif path.is_file() and path.suffix.lower() in supported:
            files.append(path)
        else:
            # Silently skip unsupported paths rather than erroring
            print(f"Warning: skipping unsupported source: {path}")
    return files


def cmd_register_source(args):
    """Register sources by content hash without extracting or publishing them."""
    from cellwiki.services.sources import SourceRegistry

    registry = SourceRegistry(settings.project_root)
    files = _collect_source_files(args.files)
    if not files:
        print("No supported source files found.")
        return
    for source_path in files:
        record = registry.register(source_path, source_type=args.source_type)
        print(f"{record.source_id}\t{record.original_name}\t{record.status.value}")


def cmd_propose_ingest(args):
    """Prepare an extraction ChangeSet; this command never publishes it."""
    from cellwiki.agent.tools import build_ingest_tools
    from cellwiki.services.sources import SourceRegistry

    registry = SourceRegistry(settings.project_root)
    prepare = {tool.name: tool for tool in build_ingest_tools(settings.project_root)}["prepare_ingest_change_set"]
    files = _collect_source_files(args.files)
    if not files:
        print("No supported source files found.")
        return
    for source_path in files:
        source = registry.register(source_path, source_type=args.source_type)
        result = prepare.invoke({"source_id": source.source_id, "run_id": args.run_id})
        print(result)


def cmd_commit_changeset(args):
    """Commit a ChangeSet only when the operator explicitly supplies --approve."""
    ...
    # Safety gate: require explicit --approve to prevent accidental commits
    if not args.approve:
        print("Refusing to commit: pass --approve for an explicit approval.")
        return
    result = CentralWriter(settings.project_root, repository=ChangeSetRepository(settings.project_root)).commit(
        args.change_set_id,
        ApprovalDecision(approved=True, decided_by=args.decided_by),
    )
...
def cmd_lint_v2(args):
    """Inspect projection state and optionally propose, but never directly apply, repairs."""
    ...
    service = LintFixService(settings.project_root)
    report = service.inspect()
    print(f"  Total issues: {report['issue_count']}")
    print(f"  Errors: {report['error_count']}")
    print(f"  Warnings: {report['warning_count']}")
    if getattr(args, "no_fix", False):
        print("  Read-only mode: no files were written.")
        return

    # Only collect findings that have deterministic auto-fix proposals
    finding_ids = [
        finding["finding_id"]
    ...
    if not finding_ids:
        print("  No deterministic repair proposals are available.")
        return
    change_set = service.propose(
        finding_ids,
        run_id=f"lint_cli_{int(time.time())}",
        max_iterations=getattr(args, "max_iterations", 3),
    )
    print(f"  Proposed ChangeSet: {change_set.change_set_id}")
    print("  Review and approve it before CentralWriter applies any repair.")


def cmd_memory(args):
    """Manage LangGraph checkpoint memory."""
    from cellwiki.legacy.graphs.checkpointer import list_threads, get_thread_history, delete_thread, cleanup_old_threads, get_db_stats, get_db_path
    action = args.action
    if action == "list":
...
def cmd_debug(args):
    ...
    action = args.action
    if action == "graph":
        from cellwiki.legacy.graphs.debug import visualize_graph
        cmd = args.command
        build_fns = {"ingest": "cellwiki.legacy.ingest_graph:build_ingest_graph", "query": "cellwiki.legacy.query_graph:build_query_graph",
                     "lint": "cellwiki.legacy.lint_graph:build_lint_graph", "research": "cellwiki.legacy.research_graph:build_research_graph",
                     "orchestrator": "cellwiki.legacy.orchestrator:build_orchestrator"}
        if cmd in build_fns:
        ...
        from cellwiki.legacy.graphs.debug import trace_execution
        cmd = args.command
        if cmd == "query":
            from cellwiki.legacy.query_graph import build_query_graph
            graph = build_query_graph()
            # Default initial state for tracing a query — avoids CLI boilerplate
            ...
            from cellwiki.legacy.lint_graph import build_lint_graph
        ...
        from cellwiki.legacy.graphs.debug import get_memory_usage
        mem = get_memory_usage()
        print("Memory Usage")
        for k, v in mem.items():
            print(f"  {k}: {v}")
    elif action == "history":
        from cellwiki.legacy.graphs.debug import visualize_state_history
        print(visualize_state_history(args.thread_id))


def main():
    parser = argparse.ArgumentParser(
        prog="cellwiki-legacy",
        description="Deprecated CellWiki CLI and LangGraph compatibility workflows",
    )
    ...
    p_register = sub.add_parser(
        "source-register",
        help="Register sources by content hash for the Agentic v1 workflow",
    )
    p_register.add_argument("files", nargs="+", type=Path)
    p_register.add_argument("--source-type", default="paper")
    p_register.set_defaults(func=cmd_register_source)

    p_propose = sub.add_parser(
        "agentic-ingest",
        help="Analyze sources and create ChangeSets without publishing",
    )
    p_propose.add_argument("files", nargs="+", type=Path)
    p_propose.add_argument("--source-type", default="paper")
    p_propose.add_argument("--run-id", default="cli-agentic-ingest")
    p_propose.set_defaults(func=cmd_propose_ingest)

    p_commit = sub.add_parser(
        "commit-changeset",
        help="Apply an approved Agentic ChangeSet",
    )
    p_commit.add_argument("change_set_id")
    p_commit.add_argument("--approve", action="store_true")
    p_commit.add_argument("--decided-by", default="local-cli-user")
    p_commit.set_defaults(func=cmd_commit_changeset)
    ...
    # Dispatch to the subcommand handler or show help if no command given
