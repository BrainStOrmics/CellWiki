"""Ingest LangGraph subgraph.

Two-step chain: Analysis -> (Review) -> Generation
...
def extract_text(state: IngestState) -> dict:
    """Extract text from PDF or read Markdown file."""
...
def build_analysis(state: IngestState) -> dict:
    ...
    for entity in analysis.get("entities", []):
        name = entity.get("standard_name", "")
        if name in existing_pages:
            # Simple contradiction detection: check if markers conflict
