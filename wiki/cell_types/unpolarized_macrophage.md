---
aliases:
- resting macrophage
- unpolarized macrophages
cl_id: null
display_name: Unpolarized Macrophage
parent_type: macrophage
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: unpolarized_macrophage
evidence_tier: 4
source_count: 1
positive_markers: []
negative_markers: []
tissues:
- in vitro culture
species:
- Mus musculus
conflicts: []
---

# Unpolarized Macrophage

> Non-polarized macrophages used as a baseline reference in computational polarization analysis.

## Functional Characteristics

- **Baseline reference for macrophage polarization scoring** (homeostatic signaling)
  - Source: [exmaple_paper_2]()

## Contexts

### Mus musculus

**Tissues:** in vitro culture

## Related Cell Types

- Parent: [macrophage](macrophage.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
