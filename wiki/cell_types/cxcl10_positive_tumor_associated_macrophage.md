---
aliases:
- CXCL10+ TAMs
cl_id: null
display_name: Cxcl10 Positive Tumor Associated Macrophage
parent_type: tumor_associated_macrophage
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: cxcl10_positive_tumor_associated_macrophage
evidence_tier: 4
source_count: 1
positive_markers:
- CXCL10
negative_markers: []
tissues:
- tumor tissue
species:
- Homo sapiens
conflicts: []
---

# Cxcl10 Positive Tumor Associated Macrophage

> A macrophage subset defined by CXCL10 expression, enriched in tumor microenvironments.

## Markers

| Marker | Type | Evidence | Source |
|--------|------|----------|--------|
| CXCL10 | positive | scRNA-seq clustering | [exmaple_paper_2]() |

## Functional Characteristics

- **Enriched in tumor tissues compared to normal tissues** (chemokine signaling)
  - Source: [exmaple_paper_2]()

## Contexts

### Homo sapiens

**Tissues:** tumor tissue

**Diseases:** multiple solid tumors

## Related Cell Types

- Parent: [tumor_associated_macrophage](tumor_associated_macrophage.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
