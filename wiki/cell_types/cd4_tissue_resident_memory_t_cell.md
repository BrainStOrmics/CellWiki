---
aliases:
- CD4_C05
- CD4_C05-CXCR6 / TRM
- TRM
cl_id: null
display_name: CD4 Tissue Resident Memory T Cell
parent_type: cd4_t_cell
references:
- paper_id: exmaple_paper_1
  title: Lineage tracking reveals dynamic relationships of T cells in colorectal cancer
standard_name: cd4_tissue_resident_memory_t_cell
---

# CD4 Tissue Resident Memory T Cell

> Tissue-resident memory CD4+ T cell cluster defined by CXCR6.

## Markers

| Marker | Type | Evidence | Source |
|--------|------|----------|--------|
| CXCR6 | positive | scRNA-seq | [exmaple_paper_1]() |

## Functional Characteristics

- **Tissue-retained CD4+ cells mediating local immune responses** (Tissue residency)
  - Source: [exmaple_paper_1]()

## Contexts

### Homo sapiens

**Tissues:** colorectal tumor, adjacent normal mucosa

**Diseases:** colorectal cancer

## Related Cell Types

- Parent: [cd4_t_cell](cd4_t_cell.md)

## References

- Lineage tracking reveals dynamic relationships of T cells in colorectal cancer (2018) [10.1038/s41586-018-0694-x](https://doi.org/10.1038/s41586-018-0694-x)
