---
aliases:
- Total immune cells
- leukocytes
cl_id: CL:0000738
display_name: Immune Cell
parent_type: null
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: immune_cell
---

# Immune Cell

> All CD45-positive leukocytes isolated from tumor tissues for comprehensive immune profiling.

## Markers

| Marker | Type | Evidence | Source |
|--------|------|----------|--------|
| Cd45 | positive | flow cytometry gating (DAPI-CD45+) | [exmaple_paper_2]() |

## Functional Characteristics

- **Represent the total leukocyte compartment in tumor digests** (general immune response)
  - Source: [exmaple_paper_2]()

## Contexts

### Mus musculus

**Tissues:** tumor

**Diseases:** cancer

## Subpopulations

- [b_cell](b_cell.md)
- [myeloid_cell](myeloid_cell.md)
- [natural_killer_cell](natural_killer_cell.md)
- [t_cell](t_cell.md)

## Related Cell Types

- [b_cell](b_cell.md)
- [myeloid_cell](myeloid_cell.md)
- [natural_killer_cell](natural_killer_cell.md)
- [t_cell](t_cell.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
