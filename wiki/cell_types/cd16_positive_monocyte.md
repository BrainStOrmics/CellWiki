---
aliases:
- Mono-CD16
- non-classical monocyte
cl_id: null
display_name: Cd16 Positive Monocyte
parent_type: monocyte
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: cd16_positive_monocyte
evidence_tier: 4
source_count: 1
positive_markers:
- FCGR3A
negative_markers: []
tissues:
- peripheral blood
- tumor tissue
species:
- Homo sapiens
conflicts: []
---

# Cd16 Positive Monocyte

> A monocyte subset characterized by CD16 expression, typically associated with non-classical monocytes.

## Markers

| Marker | Type | Evidence | Source |
|--------|------|----------|--------|
| FCGR3A | positive | scRNA-seq clustering | [exmaple_paper_2]() |

## Functional Characteristics

- **Patrolling monocytes involved in vascular surveillance** (Fc receptor signaling)
  - Source: [exmaple_paper_2]()

## Contexts

### Homo sapiens

**Tissues:** peripheral blood, tumor tissue

**Diseases:** multiple solid tumors

## Related Cell Types

- Parent: [monocyte](monocyte.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
