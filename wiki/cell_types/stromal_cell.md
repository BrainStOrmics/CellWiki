---
aliases:
- stromal cells
- tumor stroma
cl_id: CL:0000499
display_name: Stromal Cell
parent_type: connective_tissue_cell
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: stromal_cell
---

# Stromal Cell

> Non-hematopoietic structural cells in the tumor microenvironment referenced in comparative single-cell studies.

## Functional Characteristics

- **Undergo phenotype molding within the lung tumor microenvironment** (Tissue remodeling)
  - Source: [exmaple_paper_2]()

## Contexts

### Homo sapiens

**Tissues:** lung tumor

**Diseases:** lung cancer

## Related Cell Types

- Parent: [connective_tissue_cell](connective_tissue_cell.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
