---
aliases:
- Neutrophils
- PMN
cl_id: CL:0000775
display_name: Neutrophil
parent_type: myeloid_cell
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: neutrophil
evidence_tier: 4
source_count: 1
positive_markers: []
negative_markers:
- SPP1
tissues:
- peripheral blood
- tumor microenvironment
- tumor tissue
species:
- Homo sapiens
- Mus musculus
conflicts: []
---

# Neutrophil

> Granulocytic innate immune cells that show minimal SPP1 expression in the studied tumor models.

## Markers

| Marker | Type | Evidence | Source |
|--------|------|----------|--------|
| SPP1 | negative | scRNA-seq and flow cytometry | [exmaple_paper_2]() |

## Functional Characteristics

- **Innate immune cells with low SPP1 expression in this context** (innate immunity)
  - Source: [exmaple_paper_2]()
- **Exhibit anti-tumor antigen-presenting potency** (Antigen presentation pathway)
  - Source: [exmaple_paper_2]()

## Contexts

### Homo sapiens

**Tissues:** tumor tissue, peripheral blood, tumor microenvironment

**Diseases:** colorectal cancer, cancer

### Mus musculus

**Tissues:** tumor tissue, peripheral blood, tumor microenvironment

**Diseases:** colorectal cancer, cancer

## Related Cell Types

- Parent: [myeloid_cell](myeloid_cell.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
