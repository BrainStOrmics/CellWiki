---
aliases:
- LLC1
- LLC1 cells
- Lewis lung carcinoma
cl_id: null
display_name: Lung Carcinoma Cell Line
parent_type: tumor_cell
references:
- paper_id: exmaple_paper_2
  title: An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated
    macrophages and shapes an immunosuppressive tumor microenvironment
standard_name: lung_carcinoma_cell_line
---

# Lung Carcinoma Cell Line

> A murine Lewis lung carcinoma cell line used for subcutaneous tumor implantation.

## Functional Characteristics

- **Used to establish subcutaneous lung tumor models** (tumor modeling)
  - Source: [exmaple_paper_2]()

## Contexts

### Mus musculus

**Tissues:** lung

**Diseases:** lung carcinoma

## Related Cell Types

- Parent: [tumor_cell](tumor_cell.md)

## References

- An SPP1-SOCS1 pathway constrains interferon responses in tumor-associated macrophages and shapes an immunosuppressive tumor microenvironment (2026) [10.1016/j.immuni.2026.04.001](https://doi.org/10.1016/j.immuni.2026.04.001)
